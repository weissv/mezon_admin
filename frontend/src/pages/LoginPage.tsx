...
export default function LoginPage() {
  ...
  const loginSchema = z.object({
    login: z.string().min(1, t('auth.usernameRequired')),
  ...
  const sellingPoints = [
    ...
    t('landing.features.catering'),
    t('landing.features.documents'),
  ];
    ...
    <div className="relative mx-auto flex min-h-[85vh] max-w-6xl flex-col justify-center px-6">
      ...
      <div className="relative grid gap-10 rounded-[40px] bg-white/90 p-10 shadow-mezon lg:grid-cols-2">
        <div className="flex flex-col justify-between">
          <div>
            <span className="mezon-badge">{t('landing.tagline')}</span>
        ...
        <div className="mezon-card shadow-mezon">
          <div className="mb-6 text-center">
            <p className="text-sm uppercase tracking-[0.35em] text-[var(--mezon-text-soft)]">{t('auth.loginSubtitle')}</p>
            <p className="text-2xl font-semibold text-[var(--mezon-dark)]">{t('auth.loginTitle')}</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" autoComplete="off">
            <div>
              <label className="text-sm font-semibold text-[var(--mezon-dark)]" htmlFor="login">{t('auth.username')}</label>
              <Input id="login" type="text" autoComplete="off" {...register('login')} />
              <FormError message={errors.login?.message} />
            </div>
            <div>
              <label className="text-sm font-semibold text-[var(--mezon-dark)]" htmlFor="password">{t('auth.password')}</label>
