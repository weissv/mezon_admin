...
import { createCalendarEvent, fetchCalendarGroups, updateCalendarEvent } from '../../features/calendar';
import type { GroupOption } from '../../features/calendar';
...
export function EventForm({ initialData, onSuccess, onCancel }: EventFormProps) {
  ...
  useEffect(() => {
    fetchCalendarGroups()
      .then(setGroups)
      .catch((error: unknown) => {
        const message = error instanceof Error ? error.message : 'Ошибка загрузки классов';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        ...
        <select {...register('groupId')} className="mezon-field px-3 py-2">
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          />
          <Button type="button" variant="outline" onClick={addPerformer}>
            Добавить
          </Button>
        </div>
        {performers.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-2">
            {performers.map((p, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 px-2 py-1 bg-[rgba(0,122,255,0.12)] text-blue-800 rounded-full text-sm"
              >
                {p}
                <button type="button" onClick={() => removePerformer(idx)} className="hover:text-macos-red">
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
