...
const STATUS_ICONS: Record<string, { icon: typeof Clock; color: string}> = {
 ...
 delivered: { icon: Truck, color: 'text-[var(--color-green)]'},
...
export default function ProcurementStatusWidget({ data}: { data: ProcurementData | undefined}) {
 ...
 const Icon = cfg.icon;
 return (
 <div key={s.status} className="flex items-center gap-1 px-2 py-1 bg-[var(--fill-quaternary)] rounded-md text-xs">
 ...
 <p className="text-[var(--text-tertiary)]">{new Date(order.date).toLocaleDateString('ru-RU')}</p>
 </div>
