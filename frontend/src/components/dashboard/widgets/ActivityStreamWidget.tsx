...
const ACTION_ICONS: Record<string, { icon: typeof Activity; color: string}> = {
 create: { icon: Plus, color: 'text-macos-green'},
 update: { icon: FileEdit, color: 'text-blue-500'},
 delete: { icon: Trash2, color: 'text-macos-red'},
 settings: { icon: Settings, color: 'text-secondary'},
};
...
export default function ActivityStreamWidget({ data}: { data: ActivityStreamData | undefined}) {
 ...
 {entries.length === 0 && (
 <p className="text-sm text-tertiary text-center py-4">Нет активности</p>
 )}
 ...
 <span className="text-secondary">{ACTION_LABELS[entry.action] ?? entry.action}</span>{' '}
