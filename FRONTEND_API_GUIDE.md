...
1. [🚀 Раздел 1. Быстрый старт для Frontend (Quickstart)](#-раздел-1-быстрый-старт-для-frontend-quickstart)
   - [1.1. Запуск локального сервера разработки (Vite SPA)](#11-запуск-локального-сервера-разработки-vite-spa)
   - [1.2. Конфигурация переменных окружения (.env.development)](#12-конфигурация-переменных-окружения-envdevelopment)
   - [1.3. Сценарии работы с Backend](#13-сценарии-работы-с-backend)
   - [1.4. Регламент разработки и стандарты Git (GitHub Workflow)](#14-регламент-разработки-и-стандарты-git-github-workflow)
...
### 1.2. Конфигурация переменных окружения (`..env.development`)

Создайте файл `frontend/.env.development` в папке `frontend/`:
...
Укажите сетевой адрес удаленного dev-сервера в `frontend/.env.development`:
```env
VITE_API_URL=https://dev-api.mezon.uz/api
```

### 1.4. Регламент разработки и стандарты Git (GitHub Workflow)

Для обеспечения стабильности продакшн-контура и предотвращения конфликтов слияния в монорепозитории для всех frontend-разработчиков устанавливается строгий регламент работы с версиями и ветками в GitHub:

#### 🌿 Правила наименования веток (Branching Strategy)
1. **Основная ветка (`main`)**: Содержит только стабильный, протестированный код. Прямые коммиты в `main` (**`git push origin main`**) категорически запрещены.
2. **Создание изоляционных веток**: Под каждую отдельную задачу, фичу или исправление необходимо создавать отдельную рабочую ветку от актуального состояния `main`.
3. **Формат названий веток**:
   - `feature/fe-<название_задачи>` — реализация нового функционала или страницы (например: `feature/fe-login-page`, `feature/fe-billing-contracts`, `feature/fe-student-profile`).
   - `fix/fe-<название_проблемы>` — исправление багов или выравнивание верстки (например: `fix/fe-cors-credentials`, `fix/fe-table-pagination`).
   - `refactor/fe-<компонент>` — рефакторинг существующего кода без изменения функциональности (например: `refactor/fe-auth-context`).

#### 🔄 Пошаговый цикл разработки задачи (Task Workflow)

```bash
# 1. Переключение на главную ветку и получение свежих изменений
git checkout main
git pull origin main

# 2. Создание и переход в новую изолированную ветку под задачу
git checkout -b feature/fe-login-page

# 3. Выполнение работы, добавление и коммит изменений (рекомендуется Conventional Commits)
git add .
git commit -m "feat(fe-auth): add modern login page layout and zod validation"

# 4. Публикация ветки в удаленный репозиторий GitHub
git push origin feature/fe-login-page
```

#### 🔀 Открытие и проверке Pull Request (PR)
1. После отправки ветки на GitHub перейдите в интерфейс репозитория и создайте **Pull Request** из вашей рабочей ветки (`feature/fe-...`) в защищенную ветку `main`.
2. В описании PR укажите краткую суть изменений и ссылки на решаемые задачи.
3. Перед созданием PR **обязательно убедитесь в локальной валидности кода**:
   ```bash
   cd frontend
   npm run build
   ```
   *Сборка проекта Vite TypeScript должна проходить без ошибок и предупреждений типизации.*
4. В случае появления конфликтов с веткой `main` в процессе вашей разработки выполните подтягивание свежих изменений в вашу рабочую ветку:
   ```bash
   git checkout feature/fe-login-page
   git fetch origin
   git merge origin/main
   ```
   Разрешите конфликты локально, проверьте работоспособность и отправите обновленную ветку `git push origin feature/fe-login-page`.
