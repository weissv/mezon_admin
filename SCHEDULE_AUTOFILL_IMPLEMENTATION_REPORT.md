...
3. **Безопасное построение учебного плана без Декартова взрыва (Fix #3)**:
   - Исключено Декартово умножение всех учителей на все классы. Учебный план строится из таблиц `LmsSubject` и `TeacherSubject` с контролем предельной недельной нагрузки класса ($\le 35$ слотов/нед).
4. **Масочная онлайн-валидация `validateMove` (Fix #4)**:
   - В метод `validateMove` добавлена проверка индивидуальных масок доступности учителей (`teacherAvailability`) и кабинетов (`roomAvailability`).
5. **Виртуальные кабинеты и предотвращение INFEASIBLE при нехватке помещений (Fix #5)**:
   - В CP-SAT модель добавлен Виртуальный Кабинет `id: 0` ("Без кабинета") для дисциплин без жесткого привязанного помещения. Ограничение уникальности кабинета не распространяется на виртуальный кабинет `0`.
...
$$\text{Minimize} \left( w_1 \cdot \sum_{d,t,c} is\_gap_{d,t,c} + w_2 \cdot \sum_{d,t,s} is\_teacher\_gap_{d,t,s} + w_3 \cdot \sum_{d,c} overload_{d,c} \right)$$
Весовые коэффициенты: $w_1 = 10, w_2 = 10, w_3 = 20$.
